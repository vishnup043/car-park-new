...
const Dashboard: React.FC<DashboardProps> = ({ onNavigate }) => {
  ...
  const stats = useMemo(() => {
    ...
    return {
      today: jobs.filter(j => j.dateIn >= today).length,
  ...
  const recentJobs = useMemo(() => {
    return [...jobs].sort((a, b) => b.dateIn - a.dateIn).slice(0, 5);
  }, [jobs]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
          <div className="flex items-center justify-between mb-6">
            ...
            <select className="text-sm bg-gray-50 border-none rounded-lg px-3 py-1 text-gray-600">
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
              ...
              recentJobs.map(job => (
                <div key={job.id} className="flex items-center gap-4 group cursor-pointer" onClick={() => onNavigate('jobs')}>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                    job.status === JobStatus.COMPLETED ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'
                  }`}>
                    {job.status === JobStatus.COMPLETED ? <CheckCircle2 size={18} /> : <Wrench size={18} />}
                  </div>
                  <div className="flex-1 overflow-hidden">
                    <p className="text-sm font-semibold text-gray-800 truncate">Job #{job.id.slice(0, 8)}</p>
                    <p className="text-xs text-gray-500 truncate">{job.services}</p>
                  </div>
          ...
          <button 
            ...
            className="w-full mt-6 py-3 bg-blue-600 text-white font-bold rounded-xl hover:bg-blue-700 transition shadow-lg shadow-blue-200"
          >
            Create New Job
          </button>
        </div>
